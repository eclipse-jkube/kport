...
public class Config {
    ...
    public void persistToProject(RemoteDevelopmentConfig config, String projectRootFolder) {
        ...
        try {
            Path configFile = Paths.get(projectRootFolder).resolve(".kport.yaml");
            logger.info("💾 Saving kport config to file " + configFile);
    ...
    public Map<String, Map<String, RemoteDevelopmentConfig>> loadClusterBasedConfig() {
        ...
        logger.info("💾 Loading kport config from file " + configFile);
        try {
    ...
    private RemoteDevelopmentConfig loadFromLocalProject() {
        ...
        while (true) {
            Path configFile = currentDir.resolve(".kport.yaml");
            logger.info("💾 Looking for kport config from file " + configFile);
